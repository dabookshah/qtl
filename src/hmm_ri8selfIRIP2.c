 ...
 *
 * Contains: init_ri8IRIP2, emit_ri8IRIP2, step_ri8IRIP2, step_special_ri8IRIP2,
 *           calc_genoprob_ri8IRIP2, calc_genoprob_special_ri8IRIP2,
 *           argmax_geno_ri8IRIP2, sim_geno_ri8IRIP2,
 *           est_map_ri8IRIP2, nrec2_ri8IRIP2, logprec_ri8IRIP2, est_rf_ri8IRIP2,
 *           marker_loglik_ri8IRIP2, calc_pairprob_ri8IRIP2, 
 *           errorlod_ri8IRIP2, calc_errorlod_ri8IRIP2
 *
			   ...
			   double *rf, double *error_prob, double *genoprob) 
{
  calc_genoprob(*n_ind, *n_mar, 8, geno, rf, rf, *error_prob, genoprob,
			init_ri8selfIRIP2, emit_ri8selfIRIP2, step_ri8selfIRIP2);
}
			 ...
			 double *rf, double *error_prob, int *argmax)
{
  argmax_geno(*n_ind, *n_pos, 8, geno, rf, rf, *error_prob,
	      argmax, init_ri8selfIRIP2, emit_ri8selfIRIP2, step_ri8selfIRIP2);
}

void sim_geno_ri8IRIP2(int *n_ind, int *n_pos, int *n_draws, int *geno, 
		      double *rf, double *error_prob, int *draws) 
{
  sim_geno(*n_ind, *n_pos, 8, *n_draws, geno, rf, rf, *error_prob, 
	   draws, init_ri8selfIRIP2, emit_ri8selfIRIP2, step_ri8selfIRIP2);
}

/* for estimating map, must do things with recombination fractions on the RIL scale */
void est_map_ri8IRIP2(int *n_ind, int *n_mar, int *geno, double *rf, 
		     double *error_prob, double *loglik, int *maxit, 
		     double *tol, int *verbose)
{
  est_map(*n_ind, *n_mar, 8, geno, rf, rf, *error_prob, 
	  init_ri8selfIRIP2, emit_ri8selfIRIP2, step_ri8selfIRIP2, nrec_bc, nrec_bc,
...
{
  est_rf(*n_ind, *n_mar, geno, rf, nrec2_ri8selfIRIP2, logprec_ri8selfIRIP2, 
	 *maxit, *tol, 1);
}
...
{
  marker_loglik(*n_ind, 8, geno, *error_prob, init_ri8selfIRIP2, emit_ri8selfIRIP2,
		loglik);
}
			   ...
			   double *genoprob, double *pairprob) 
{
  calc_pairprob(*n_ind, *n_mar, 8, geno, rf, rf, *error_prob, genoprob,
		pairprob, init_ri8selfIRIP2, emit_ri8selfIRIP2, step_ri8selfIRIP2);
}

double errorlod_ri8selfIRIP2(int obs, double *prob, double error_prob)
{
  double p=0.0, temp;
  int n=0, i;
			   ...
			   double *errlod)
{
  calc_errorlod(*n_ind, *n_mar, 8, geno, *error_prob, genoprob,
		errlod, errorlod_ri8selfIRIP2);
}

/* end of hmm_ri8selfIRIP2.c */

 ...
 *
 * Contains: init_ri8IRIP3, emit_ri8IRIP3, step_ri8IRIP3, step_special_ri8IRIP3,
 *           calc_genoprob_ri8IRIP3, calc_genoprob_special_ri8IRIP3,
 *           argmax_geno_ri8IRIP3, sim_geno_ri8IRIP3,
 *           est_map_ri8IRIP3, nrec2_ri8IRIP3, logprec_ri8IRIP3, est_rf_ri8IRIP3,
 *           marker_loglik_ri8IRIP3, calc_pairprob_ri8IRIP3, 
 *           errorlod_ri8IRIP3, calc_errorlod_ri8IRIP3
 *
...
double init_ri8selfIRIP3(int true_gen, int *cross_scheme)
{
  return(-3.0*M_LN2); /* log(1/8) */
}

double emit_ri8selfIRIP3(int obs_gen, int true_gen, double error_prob, int *cross_scheme)
{
...
double step_ri8selfIRIP3(int gen1, int gen2, double rf, double junk, int *cross_scheme) 
{
  //See Teuscher and Broman 2007, "Haplotype Probabilities for Multiple-Strain Recombinant Inbred Lines", Equation (3), s=1
  const int s = 3;
  double equalProb = pow(1-rf, 3 + s - 1) +(2*rf + 1 - pow(1 - rf, s-1))/8;
...
void calc_genoprob_ri8selfIRIP3(int *n_ind, int *n_mar, int *geno, 
			   double *rf, double *error_prob, double *genoprob) 
{
  calc_genoprob(*n_ind, *n_mar, 8, geno, rf, rf, *error_prob, genoprob,
			init_ri8selfIRIP3, emit_ri8selfIRIP3, step_ri8selfIRIP3);
}

void calc_genoprob_special_ri8selfIRIP3(int *n_ind, int *n_mar, int *geno, 
				   double *rf, double *error_prob, double *genoprob) 
{
...
void argmax_geno_ri8selfIRIP3(int *n_ind, int *n_pos, int *geno,
			 double *rf, double *error_prob, int *argmax)
{
  argmax_geno(*n_ind, *n_pos, 8, geno, rf, rf, *error_prob,
	      argmax, init_ri8selfIRIP3, emit_ri8selfIRIP3, step_ri8selfIRIP3);
}

void sim_geno_ri8IRIP3(int *n_ind, int *n_pos, int *n_draws, int *geno, 
		      double *rf, double *error_prob, int *draws) 
{
  sim_geno(*n_ind, *n_pos, 8, *n_draws, geno, rf, rf, *error_prob, 
	   draws, init_ri8selfIRIP3, emit_ri8selfIRIP3, step_ri8selfIRIP3);
}
...
double nrec2_ri8selfIRIP3(int obs1, int obs2, double rf, int *cross_scheme)
{
  ...
  const int s = 3;
  rf0 = (pow(1-rf, 3 + s - 1)+(2*rf + 1 - pow(1 - rf, s-1))/8)/ (8*(1+2 * rf));
...
double logprec_ri8selfIRIP3(int obs1, int obs2, double rf, int *cross_scheme)
{
  int n1, n2, n12, nr, and, i, nstr=8;
  double rf0, rf1;
...
void est_rf_ri8selfIRIP3(int *n_ind, int *n_mar, int *geno, double *rf, 
		   int *maxit, double *tol)
{
  est_rf(*n_ind, *n_mar, geno, rf, nrec2_ri8selfIRIP3, logprec_ri8selfIRIP3, 
	 *maxit, *tol, 1);
}

void marker_loglik_ri8selfIRIP3(int *n_ind, int *geno,
			   double *error_prob, double *loglik)
{
  marker_loglik(*n_ind, 8, geno, *error_prob, init_ri8selfIRIP3, emit_ri8selfIRIP3,
		loglik);
}

void calc_pairprob_ri8selfIRIP3(int *n_ind, int *n_mar, int *geno, 
			   double *rf, double *error_prob, 
			   double *genoprob, double *pairprob) 
{
  calc_pairprob(*n_ind, *n_mar, 8, geno, rf, rf, *error_prob, genoprob,
		pairprob, init_ri8selfIRIP3, emit_ri8selfIRIP3, step_ri8selfIRIP3);
}

double errorlod_ri8selfIRIP3(int obs, double *prob, double error_prob)
{
  double p=0.0, temp;
  int n=0, i;
...
void calc_errorlod_ri8selfIRIP3(int *n_ind, int *n_mar, int *geno, 
			   double *error_prob, double *genoprob, 
			   double *errlod)
{
  calc_errorlod(*n_ind, *n_mar, 8, geno, *error_prob, genoprob,
		errlod, errorlod_ri8selfIRIP3);
}

/* end of hmm_ri8selfIRIP3.c */
